...
**交付**
- 统一日志接口（如 `src/shared/utils/logger.ts`）
- 替换 `console.log/warn/error` 为统一接口
- 开发环境输出详细日志，生产环境仅错误级别
...
**交付**
- ErrorBoundary 提供友好错误信息与恢复选项（重试/重置）
- 记录错误到 Terminal 面板
...
- 关键操作添加性能标记（打开项目、保存、Canvas 渲染）
- 开发环境显示性能指标
- 记录慢操作到 Terminal
- DoD：能识别性能瓶颈，开发环境可见性能指标
...
- 检查并修复所有 `any` 类型
- 为 File System Access API 补充完整类型定义
- 使用更严格的 TypeScript 配置
- DoD：无 `any` 类型，类型定义完整

#### P3-3 文档完善（待启动）

- 为复杂算法添加注释（如 Canvas 坐标转换等）
- 补充关键函数的 JSDoc
- 更新 `docs/architecture.md` 示例代码
- DoD：关键逻辑有清晰注释，文档示例可用
...
- 提取用户可见文本到 `src/shared/i18n/`
- 使用简单的 i18n 方案
- 为未来多语言做准备
- DoD：用户可见文本可配置，支持多语言扩展
